...
class PandaEnv(gym.Env):
    ...
    def step(self, action):
        ...
        def dist(a, b, echo=False):
            #print('a',a)
            #print('b',b)
            aa = np.array(a)
            bb = np.array(b)
            d = np.sum(np.abs(aa - bb)**2,axis=0)
            dr = d # + random.uniform(-0.001,+0.001)
            if echo: print(f'A:{aa}\nB:{bb}\nD:{d}\nDR:{dr}')
            return dr
        ...
        if self.goal == "bumps":
            if obj_dist > 0.005:
                reward = 1 + (obj_dist * 10)
                print(f"BUMPED! {reward}")
                done = True
                self.n_goals += 1
            else:
                # print(np.array(state_object) - np.array(newPosition))
                # newPosition = self.object_loc
                newPosition[2] += ROBOT_Z_OFS
